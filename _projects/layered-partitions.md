...
The preceding argument actually shows something considerably stronger.  The * Let $G_n$ denote the $n\times n + r$ vertex graph described above.  For any integer $r$ and any bounded-degree graph $R$, there exists an $n$ such that $G_n\not\subseteq H\boxtimes R + K_r$.  This includes the interesting case $R=P\boxtimes P\boxtimes\cdots\boxtimes P$ where the number of factors in the product is a constant.  The key observation is that $\psi$ must take $N_G[a]$ onto a subgraph of $H\boxtimes R + K_r$ of diameter at most $2$. The bounded degree of $R$ ensures that this subgraph either includes a vertex of $K_r$ or has bounded treewidth.
